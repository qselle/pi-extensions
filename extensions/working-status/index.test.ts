...
function harness(mode = "tui") {
  ...
  return { messages, indicators, entries, notifications, ctx, command: (args: string) => commands.get("working-style").handler(args, ctx), fire: (name: string, event = {}) => handlers.get(name)?.(event, ctx) };
}
...
test("branch navigation clears the previous style and stale working state", async () => {
  ...
  try {
    ...
    expect(h.notifications.at(-1)).toContain("Working style: native");
  } finally { h.fire("session_shutdown"); }
});
