...
test("host and access diagnostics use metadata without printing raw config or resolving credentials", async () => {
  ...
  const valid = await diagnose({ ...input, hostVersion: "0.87.0", modelAuthConfigured: true, modelConfigInvalid: false, bindingConflicts: 0, env: { EXA_API_KEY: "present-but-unused" } }, good);
