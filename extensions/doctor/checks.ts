...
export async function diagnose(input: DoctorInput, probe = localProbe(input)): Promise<Finding[]> {
  ...
  if (input.hostVersion) add("host-version", "Pi host", /^0\.86\.[1-9]\d*$/.test(input.hostVersion) ? "ok" : "warn", `Version ${input.hostVersion}; supported line is 0.86.x (minimum 0.86.1).`, /^0\.86\.[1-9]\d*$/.test(input.hostVersion) ? undefined : "Use the supported Pi host line and reload.");
